...
bool receive_command2(String &string_buffer){
...
void select_delay2(const String &str) {
...
void process_command2(const String &string_buffer, Ezo_board device_list[], uint8_t device_list_len, Ezo_board* &default_board){
    Serial.println(string_buffer);
  ...
  //the all command sends a command to all available boards and shows their responses
    ...
    select_delay2(cmd);                                                        //wait for some time depending on the command
  ...
  // all other commands are passed through to the default board unless they have an address prepended
    ...
    if (index != -1) {                                              //if it contains a colon character
      ...
      if (name_to_find.length() != 0) {                                              //if its valid
        ...
        if (addr_found) {                                           //then send the rest of the command to that board
            ...
            Serial.println("Sende Command");  // Bernd 
            Serial.println(string_buffer.substring(index + 1).c_str());  // Bernd
    ...
    select_delay2(string_buffer);                                   //wait for some time depending on the command

    receive_and_print_response(*default_board);
  }
}

void list_devices2(Ezo_board device_list[], uint8_t device_list_len, Ezo_board* default_board) {
